...
@section('content')
    <div class="card mb-5 mb-xl-8">
        ...
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bolder fs-3 mb-1">الطلبات الحالية</span>
            </h3>
        </div>
        <!--end::Header-->
        <!--begin::Body-->
            ...
            <div class="table-responsive">
                ...
                <table class="table table-bordered" id="users-table">
                    ...
                    <tr class="fw-bolder text-muted bg-light">
                        ...
                        <th class="min-w-50px">وسيلة الدفع</th>
                        <th class="min-w-50px">حالة الدفع</th>
    ...
    <!-------------------------Start EDIT Form --------------------------------------------------------------->
    <div class="modal fade" id="order_details_model" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content">
                <div class="modal-header">
                    <h3>تفاصيل الطلب</h3>
...
@section('js')
    ...
    <script>
        $(document).on('click', '#order_details', function (event) {
    ...
    <script>
        ...
        $(document).on('click', '#making_order', function (event) {
            var id = $(this).data("id")
            $.ajax({
                type: 'POST',
                url: "{{route('making_order')}}",
                data: {
                    '_token': "{{csrf_token()}}",
                    'id': id,
                },
                success: function (data) {
                    if (data.success === true) {
                        $('#users-table').DataTable().ajax.reload();
                        $('<script> notif({msg: "تم قبول الطلب :جارى الان تحضير الطلب",type: "success"})</' + 'script>').appendTo(document.body);
                    }
                }
            });
        });
    ...
    <script>
        // Make Better Using Ajax
        $(document).on('click', '#delivry_order', function (event) {
            var id = $(this).data("id")
            $.ajax({
                type: 'POST',
                url: "{{route('delivry_order')}}",
                data: {
                    '_token': "{{csrf_token()}}",
                    'id': id,
                },
                success: function (data) {
                    if (data.success === true) {
                        $('#users-table').DataTable().ajax.reload();
                        $('<script> notif({msg: "تم انهاء التحضير  :جارى الان توصيل الطلب",type: "success"})</' + 'script>').appendTo(document.body);
                    }
                }
            });
        });
    ...
    <script>
        // Make Better Using Ajax
        $(document).on('click', '#ending_order', function (event) {
            var id = $(this).data("id")
            $.ajax({
                type: 'POST',
                url: "{{route('ending_order')}}",
                data: {
                    '_token': "{{csrf_token()}}",
                    'id': id,
                },
                success: function (data) {
                    if (data.success === true) {
                        $('#users-table').DataTable().ajax.reload();
                        $('<script> notif({msg: "تم انهاء التوصيل  :تم توصيل الطلب الى العميل بنجاح",type: "success"})</' + 'script>').appendTo(document.body);
                    }
                }
            });
        });
    ...
    <script>
        // Make Better Using Ajax
        $(document).on('click', '#refusal_order', function (event) {
            var id = $(this).data("id")
            $.ajax({
                type: 'POST',
                url: "{{route('refusal_order')}}",
                data: {
                    '_token': "{{csrf_token()}}",
                    'id': id,
                },
                success: function (data) {
                    if (data.success === true) {
                        $('#users-table').DataTable().ajax.reload();
                        $('<script> notif({msg: "تم رفض الطلب",type: "error"})</' + 'script>').appendTo(document.body);
                    }
                }
            });
        });
